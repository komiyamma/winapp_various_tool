...
old_photo_list = []
new_photo_list = []
for jpg in p_temp.glob("**/*"):
    m = re.search(r"IMG_(\d\d\d\d\d\d\d\d)\.jpg", str(jpg), re.I)
    if m:
        old_photo_list.append(int(m.group(1)))
    else:
        m2 = re.search(r"(IMG_\d{1,6}\.jpg)", str(jpg), re.I)
        if m2:
...
my_glob = p_temp.glob("*/*.*")
for m in my_glob:
    m2 = re.search(r"(IMG_\d{1,6}\.jpg)", str(m), re.I)
    if m2:
